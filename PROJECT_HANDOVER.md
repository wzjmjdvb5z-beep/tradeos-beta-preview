...
## Screen loading follow-up — 14 September 2026

Ben says web feels cleaner but still has many loading screens, and installed app update is not visible. Build20 run34810282914 confirmed completed including Apple validation/upload; availability to tester group remains unverified (TestFlight, not public App Store release). Narrowed commercial loadContext by requested view: Quotes fetches quotes/customers/jobs without invoices/items/payments/business-profile/profitability (five requests avoided); Finance skips quotes and starts profile/profit RPCs with table requests in parallel rather than serially afterward. Employee role returns empty commercial context before any commercial requests. No session/financial caching introduced. New test-commercial-loading.cjs verifies query boundaries, concurrent RPC start and employee exclusion; commercial and employee regressions pass. Publishing screen-loading-1; delivery/build completion pending. This addresses commercial loading only, not all app screens or native availability.

## Employee identity — 14 September 2026

Ben asks to identify employees instead of generic Employee label. accept_my_company_invitations inserts full_name NULL; frontend falls back to role. Added missing-name gate for existing signed-in members and newly accepted invitations; shows account email privately and asks for a 2–100 character name, rejecting generic role placeholders. Existing update_my_membership_name RPC saves self in active company, then local own name data is updated; Team has Edit my name. Pricing list fallback now Name not set. No actual names guessed or user memberships renamed by tools. SQL rollback test confirms employee own-name update/role preservation and unrelated-company denial; no retained test data. JS test-member-name plus employee/commercial regressions pass; CI includes new test. Existing partner needs to enter their name once on updated web/native app, then Ben reloads to see it across name-based displays. Publishing employee-names-1; deployment/new TestFlight build pending.


## Veystead and reported UI failures — 14 September 2026
