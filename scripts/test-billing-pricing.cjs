...
assert.match(ui,/action:'sync_seats'/);
assert.match(ui,/await syncSeats\(ctx\.companyId\)/);
...
assert.match(checkout,/action === "sync_seats"/);
assert.match(checkout,/item\?\.price\?\.id === SEAT_PRICE/);
assert.match(checkout,/seatForm\.set\("proration_behavior", "none"\)/);
assert.match(checkout,/"DELETE", seatForm/);
assert.match(checkout,/seatForm\.set\("quantity", String\(additionalUsers\)\)/);
