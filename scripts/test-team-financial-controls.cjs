...
async function testRateSave(){
  ...
  const sb={auth:{},from(){throw new Error('Rate save must not update the membership row directly');},rpc:async(name,args)=>{calls.push({name,args});return {error:null};}};
  ...
  assert.equal(JSON.stringify(calls),JSON.stringify([{name:'set_member_cost_rate',args:{target_company:'company',target_member:'member',target_rate:18.5}}]));
