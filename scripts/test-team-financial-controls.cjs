...
(async()=>{
  ...
  const secureFix=fs.readFileSync('app/team-rates-secure-fix.js','utf8');
  ...
  assert.match(main,/window\.veysteadSupabase=sb/);
  assert.match(secureFix,/window\.veysteadSupabase\|\|/);
