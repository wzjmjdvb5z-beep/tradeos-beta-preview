(()=>{
  ...
  async function printInvoice(invoice,job,customer,items,payments,company={}){
    company=company||{};
    const paid=sum(payments.map(p=>num(p.amount))),outstanding=Math.max(0,num(invoice.total)-paid);
    const lines=items.length?items:[{description:job?.title||'Work completed',quantity:1,unit_price:num(invoice.subtotal),vat_rate:num(invoice.subtotal)?num(invoice.vat)/num(invoice.subtotal)*100:0,line_subtotal:num(invoice.subtotal),line_total:num(invoice.total)}];
    const html=`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(invoice.invoice_number||'Invoice')}</title><style>body{font:15px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#172033;margin:0;background:#eef2f6}.paper{box-sizing:border-box;max-width:820px;min-height:100vh;margin:0 auto;padding:52px;background:#fff}header{display:flex;justify-content:space-between;gap:24px;border-bottom:3px solid #0f766e;padding-bottom:24px}h1{font-size:34px;margin:0 0 6px}.brand{font-size:22px;font-weight:800;color:#0f766e}.muted{color:#667085}.grid{display:grid;grid-template-columns:1fr 1fr;gap:28px;margin:30px 0}.label{display:block;text-transform:uppercase;letter-spacing:.08em;font-size:11px;font-weight:800;color:#667085;margin-bottom:8px}table{width:100%;border-collapse:collapse;margin:22px 0}th,td{text-align:left;padding:12px 8px;border-bottom:1px solid #e5e7eb}th{font-size:11px;text-transform:uppercase;color:#667085}th:last-child,td:last-child{text-align:right}.totals{margin-left:auto;width:min(360px,100%)}.totals div{display:flex;justify-content:space-between;padding:8px}.totals .grand{font-size:20px;font-weight:800;border-top:2px solid #172033;margin-top:6px}.balance{margin-top:26px;padding:16px;background:#ecfdf5;border-radius:10px;font-weight:700;display:flex;justify-content:space-between}@media(max-width:600px){.paper{padding:28px 20px}.grid{grid-template-columns:1fr}header{display:block}.brand{margin-top:18px}}@media print{body{background:#fff}.paper{max-width:none;padding:28px 36px}@page{margin:10mm}}</style></head><body><main class="paper"><header><div><span class="label">Invoice</span><h1>${esc(invoice.invoice_number||'Invoice')}</h1><div class="muted">Created ${esc(formatDate(invoice.created_at))} · Due ${esc(invoice.due_date?formatDate(invoice.due_date):'on receipt')}</div></div><div><div class="brand">${esc(company.name||'Trade business')}</div>${[company.address,company.contact_email,company.contact_phone].filter(Boolean).map(v=>`<div class="muted">${esc(v)}</div>`).join('')}${company.company_number?`<div>Company no. ${esc(company.company_number)}</div>`:''}${company.vat_number?`<div>VAT no. ${esc(company.vat_number)}</div>`:''}</div></header><section class="grid"><div><span class="label">Invoice to</span><strong>${esc(customer?.name||'Customer')}</strong>${customer?.address?`<div class="muted">${esc(customer.address)}</div>`:''}${customer?.email?`<div class="muted">${esc(customer.email)}</div>`:''}</div><div><span class="label">For</span><strong>${esc(job?.title||'Job')}</strong>${job?.address?`<div class="muted">${esc(job.address)}</div>`:''}</div></section><table><thead><tr><th>Description</th><th>Qty</th><th>Unit</th><th>Total</th></tr></thead><tbody>${lines.map(x=>`<tr><td>${esc(x.description||'Work')}</td><td>${esc(num(x.quantity)||1)}</td><td>${money.format(num(x.unit_price))}</td><td>${money.format(num(x.line_total))}</td></tr>`).join('')}</tbody></table><div class="totals"><div><span>Net</span><strong>${money.format(num(invoice.subtotal))}</strong></div><div><span>VAT</span><strong>${money.format(num(invoice.vat))}</strong></div><div class="grand"><span>Total</span><strong>${money.format(num(invoice.total))}</strong></div>${paid>0?`<div><span>Paid</span><strong>${money.format(paid)}</strong></div>`:''}</div><div class="balance"><span>Amount due</span><strong>${money.format(outstanding)}</strong></div>${[company.bank_account_name,company.bank_sort_code,company.bank_account_number].some(Boolean)?`<section><h2>Payment details</h2>${company.bank_account_name?`<div>Account: ${esc(company.bank_account_name)}</div>`:''}${company.bank_sort_code?`<div>Sort code: ${esc(company.bank_sort_code)}</div>`:''}${company.bank_account_number?`<div>Account number: ${esc(company.bank_account_number)}</div>`:''}<div>Reference: ${esc(invoice.invoice_number)}</div></section>`:''}${company.invoice_terms?`<p style="white-space:pre-wrap">${esc(company.invoice_terms)}</p>`:''}</main></body></html>`;
    if(window.tradeOSNative?.isNative||window.Capacitor?.isNativePlatform?.()){
      try{
        if(!window.tradeOSNative?.exportInvoice)throw new Error('Update the app in TestFlight to export invoice PDFs.');
        await window.tradeOSNative.exportInvoice({html,title:invoice.invoice_number||'Invoice'});
      }catch(error){toast(error?.message||'Could not export this invoice. Please try again.',true);}
      return;
    }
    const popup=window.open('','_blank');
    if(!popup){toast('Allow pop-ups in Safari to print or save this invoice.',true);return;}
    popup.document.open();
    popup.document.write(html);
    // Register from this external script: the inherited CSP forbids inline scripts.
    ...
    retry.type='button';retry.textContent='Print / Save PDF';
    retry.className='print-action';retry.addEventListener('click',print);
    popup.document.body.prepend(retry);
    const printStyle=popup.document.createElement('style');
    printStyle.textContent='.print-action{display:block;margin:16px auto;padding:12px 20px}@media print{.print-action{display:none}}';
  ...
  window.addEventListener('tradeos:refresh-workflow',()=>{
    const view=currentView();
    if(view==='quotes'||view==='finance')reload(view);
  });
  ...
  function displayInvoiceStatus(i,out){if(i.status==='void')return 'void';if(i.status==='paid'||out<=.005)return 'paid';if(i.due_date&&new Date(`${i.due_date}T23:59:59`).getTime()<Date.now())return 'overdue';return i.status||'draft';}
