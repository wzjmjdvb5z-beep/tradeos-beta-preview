(()=>{
  ...
  function renderToday(jobs,todayIso){
    ...
    return `<section class="tos-today-list">${dayJobs.length?dayJobs.map(j=>{const people=jobPeople(j.id);return `<div class="tos-today-card ${ctx.isManager?'clickable':''}" ${ctx.isManager?`data-schedule-job="${esc(j.id)}"`:''}><div class="tos-today-time">${timeRange(j)}</div><div><h4>${esc(j.title)}</h4><div class="tos-today-meta">${esc(j.address||'No address')} · ${esc(j.status||'booked')}</div>${people.length?`<div class="tos-assignees">${people.map(p=>`<span class="tos-person-chip">${esc(p.full_name||'Team member')}</span>`).join('')}</div>`:''}</div></div>`}).join(''):`<section class="card section"><div class="empty">Nothing scheduled for today.</div></section>`}</section>`;
  }

  function scheduleJobCard(j){
    const people=jobPeople(j.id);
    return `<div class="tos-schedule-job ${ctx.isManager?'clickable':''}" ${ctx.isManager?`data-schedule-job="${esc(j.id)}"`:''}><div class="tos-job-time"><strong>${timeRange(j)}</strong><span class="tos-job-status">${esc(j.status||'booked')}</span></div><div class="tos-job-title">${esc(j.title)}</div>${j.address?`<div class="tos-job-address">${esc(j.address)}</div>`:''}${people.length?`<div class="tos-assignees">${people.map(p=>`<span class="tos-person-chip">${esc(p.full_name||'Team member')}</span>`).join('')}</div>`:''}</div>`;
  }
  ...
  async function saveSchedule(job,overlay){
    ...
    const date=overlay.querySelector('#tos-job-date')?.value,startTime=overlay.querySelector('#tos-job-start')?.value,endTime=overlay.querySelector('#tos-job-end')?.value,status=overlay.querySelector('#tos-job-status')?.value||'booked';
    ...
    try{
      ...
      const update=await client.from('jobs').update({scheduled_start:start.toISOString(),scheduled_end:end.toISOString(),status}).eq('company_id',ctx.companyId).eq('id',job.id);
