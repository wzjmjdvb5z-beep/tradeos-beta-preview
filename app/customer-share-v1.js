(()=>{
  const SUPABASE_URL='https://nynssdxfmjfqgodgynnu.supabase.co',KEY='sb_publishable_ose18MeKd0ZPfTM1tbq2fg_hzfVcTxf',client=window.supabase?.createClient(SUPABASE_URL,KEY,{auth:{persistSession:true,autoRefreshToken:false,detectSessionInUrl:false}});
  if(!client)return;
  ...
  async function makeLink(type,id,button){
    ...
    try{const r=await client.rpc('create_document_share_token',{target_type:type,target_id:id,expires_days:90});if(r.error)throw r.error;const token=r.data;const base=new window.URL(type==='quote'?'../quote/':'../invoice/',window.location.href);base.searchParams.set('t',token);openShare(type,base.href);updateStatus(type,id);const el=document.querySelector('.tos-com-sheet [data-share-delivery]');if(el){el.className='tos-share-status sent';el.textContent='Sent · not viewed yet';}}
  ...
  function openShare(type,url){
    ...
    const close=()=>o.remove();o.querySelector('.tos-share-close').addEventListener('click',close);o.addEventListener('click',e=>{if(e.target===o)close();});o.querySelector('[data-share-copy]').addEventListener('click',async e=>{try{await navigator.clipboard.writeText(url);e.currentTarget.textContent='Copied';}catch{fallbackCopy(url);e.currentTarget.textContent='Copied';}});o.querySelector('[data-share-native]').addEventListener('click',async()=>{if(navigator.share){try{await navigator.share({title:type==='quote'?'TradeOS quote':'TradeOS invoice',text:type==='quote'?'Here is your quote.':'Here is your invoice.',url});}catch{}}else{fallbackCopy(url);showToast('Link copied');}});o.querySelector('[data-share-preview]').addEventListener('click',()=>window.location.assign(url));
  }
