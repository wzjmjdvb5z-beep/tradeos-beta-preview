(()=>{
  ...
  const sb=window.supabase?.createClient(URL,KEY,{auth:{persistSession:true,autoRefreshToken:false,detectSessionInUrl:false}});
  if(!sb)return;
  ...
  async function mount(){
    ...
    if(!form.querySelector('.tos-employee-form-note'))form.insertAdjacentHTML('afterbegin','<div class="tos-employee-form-note"><strong>Add their account email</strong><span>They must create their TradeOS account using this exact email address.</span></div>');
  ...
  async function invite(form){
    ...
    if(!email||!email.includes('@')){showToast('Enter your partner’s email address.',true);return;}
  ...
  async function shareInvite(email=''){
    const url=employeeUrl(),text=`Open TradeOS, tap Create account and use ${email||'the email address I invited'}. Confirm the email, then sign in to see the employee view.`;
    try{if(window.tradeOSNative?.share){await window.tradeOSNative.share({title:'Join my TradeOS team',text,url});return;}if(navigator.share){await navigator.share({title:'Join my TradeOS team',text,url});return;}await navigator.clipboard.writeText(`${text}\n${url}`);showToast('Employee instructions copied');}
    catch(err){if(err?.name!=='AbortError')showToast('Could not share the link.',true);}
  }
  function customiseJoinScreen(){
    if(new URLSearchParams(location.search).get('join')!=='employee')return;const auth=document.querySelector('.auth-card #auth');if(!auth||auth.dataset.employeeJoin==='1')return;auth.dataset.employeeJoin='1';const h=auth.parentElement.querySelector('h2'),p=auth.parentElement.querySelector('.sub');if(h)h.textContent='Join your TradeOS team';if(p)p.textContent='Create your employee account using the exact email address your employer invited.';const signup=auth.querySelector('#signup');if(signup){signup.textContent='Create employee account';signup.classList.remove('secondary');}const signIn=auth.querySelector('button:not([type])');if(signIn)signIn.textContent='Already registered? Sign in';
  }
